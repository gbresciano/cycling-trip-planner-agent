...
export function createOrchestrator(deps: OrchestratorDeps): Orchestrator {
  ...
  // The one place the orchestrator couples itself to a specific tool:
  // without `submit_trip_plan` the agent has no way to commit a plan
  // back to us. Registered idempotently so multiple orchestrators sharing
  // a registry don't collide.
  ...
  return {
    ...
    sendMessage: (state, userInput) => runTurn(state, userInput, deps, {
      systemPrompt,
      maxIterations,
      now,
    }),
...
async function runTurn(
  ...
  let working: ConversationState = appendMessage(
    state,
    makeMessage("user", [{ type: "text", text: userInput }], opts.now()),
    { status: "planning", now: opts.now },
  );
  ...
  for (let i = 0; i < opts.maxIterations; i++) {
    ...
    working = appendMessage(
      working,
      makeMessage("assistant", assistantContent, opts.now()),
      { now: opts.now },
    );
    ...
    working = appendMessage(
      working,
      makeMessage("user", toolResults, opts.now()),
      {
        plan,
...
// Tool dispatch.
//
// `submit_trip_plan` is special: instead of returning external data, it
// signals "I'm done — here is the structured plan." We re-parse its
// input through the zod schema (defense in depth — the registry would
// too) and build the canonical `TripPlan` with computed totals.
//
...
function buildTripPlan(input: SubmitTripPlanInput): TripPlan {
  const totalDistanceKm = input.segments.reduce(
    (sum, s) => sum + s.route.distanceKm,
    0,
  );
  const totalElevationGainM = input.segments.reduce(
    (sum, s) => sum + s.route.elevation.totalGainM,
    0,
  );
  return {
    id: randomUUID(),
...
function assistantBlocksToContent(
